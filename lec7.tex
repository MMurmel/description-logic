...
\begin{example}[Intuition]
	...
	\begin{figure}[H]
		\centering
		\begin{tikzpicture}
			\node[default, label= left: $\left\{ B \right\}$] (e) {$e$};
			\node[default, label= right: $\emptyset$, right of = e] (f) {$f$};
			\node[default, label= above: $\left\{ A \right\}$, above right of = e] (d) {$d$};
			\draw (e) edge[above, bend left] node{$r$} (d);
			\draw (d) edge[left, bend left] node{$r$} (e);
